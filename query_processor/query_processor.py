    ...
    def execute_query(self, query: str, client_id: int):
        ...
        if query_type == "BEGIN_TRANSACTION":
            ...
            if query_type == "COMMIT":
                # COMMIT no longer writes to storage directly
                # It only logs commit and triggers checkpoint
                print(f"\n[COMMIT] Committing transaction {t_id}...")
                buffered_ops = self.query_execution_engine.transaction_buffer.get_buffered_operations(t_id)
                print(f"[COMMIT] Transaction has {len(buffered_ops)} buffered operation(s)")
                
                # Log commit to FRM (this will trigger checkpoint which flushes to storage)
                self.adapter_frm.log_commit(t_id)
                
                # Commit transaction in CCM
                self.adapter_ccm.commit_transaction(t_id)
                
                print(f"[COMMIT] Transaction {t_id} committed and checkpoint triggered")
            else:
            ...
            try:
                ...
                if not in_explicit_transaction:
                    ...
                    buffered_ops = self.query_execution_engine.transaction_buffer.get_buffered_operations(t_id)
                    print(f"[AUTO-COMMIT] Transaction has {len(buffered_ops)} buffered operation(s)")
                    
                    # Log commit (this triggers checkpoint which flushes to storage)
                    self.adapter_frm.log_commit(t_id)
                    self.adapter_ccm.commit_transaction(t_id)
                    print(f"[AUTO-COMMIT] Transaction {t_id} committed and checkpoint triggered")
